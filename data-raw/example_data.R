...
data_continuous_48_incomplete <- data.frame(matrix(c(round(abs(rnorm(3)), 2),
                                                     NA,
                                                     round(abs(rnorm(20)), 2),
                                                     rep(NA, 24)), nrow = 6, ncol = 8)) |>
  mutate(rows = LETTERS[1:6]) |>
  pivot_longer(cols = -rows, names_to = "cols", values_to = "Value") |>
  mutate(cols = as.numeric(str_remove(cols, pattern = "V|X")),
         well = paste0(rows, cols)) |>
...
data_discrete_96 <- data.frame(matrix(c("Positive Control",
                                        ...
                                        "Negative Control", "Negative Control"), nrow = 8, ncol = 12)) |>
  mutate(rows = LETTERS[1:8]) |>
