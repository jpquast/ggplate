...
The **ggplate** package is archived on Zenodo, which provides a DOI that can be used to cite it:

Jan-Philipp Quast. ggplate: Create Layout Plots of Biological Culture Plates and
Microplates. *Zenodo*, 2026, <https://doi.org/10.5281/zenodo.22228623>

Please make sure to cite **ggplate** if you used it to create figures for your
publication. The citation of the version you are using can always be retrieved
with `citation("ggplate")`.
...
When calling the `str()` function you can see that the data frame of a continuous 96-well plate dataset contains three columns. The `Value` column contains values associated with each of the plate wells, while the `well` column contains the corresponding well positions using a combination of **alphabetic row names** and **numeric column names**. The `Status` column contains a category for some of the wells, which we will use later to colour the borders of these wells.
...
If you specifically want to keep `NA` values in the plot, you can set the `remove_na` argument to `FALSE`. You can also specifically control the fill colour of `NA` values with the `na_fill` argument.

```{r standard_plot_48_empty_wells_keep_na, dpi=260, fig.width=7, fig.height=4}
...
In addition to the fill colour of a well you can also colour its border. This makes it possible to highlight individual wells, for example those that did not pass a quality control. The `border` argument takes the name of a column that contains categories. Wells that contain `NA` in this column keep their default black border and are not shown in the legend.

The `Status` column of the 96-well dataset marks wells with a low signal. Its categories are shown in a second legend below the legend of the `value` argument.

```{r well_borders, dpi=260, fig.width=7, fig.height=4}
...
The border colours are taken from the `border_colours` scheme, which contains four colours that are not part of the colour schemes used for the fill of the wells. You can provide your own colours to the `border_colour` argument. This is also necessary if your column contains more categories than the four colours of the default scheme.

The `Replicate` column of the discrete 96-well dataset contains the number of the replicate of each compound. Since the two controls have eight replicates, eight colours have to be provided to the `border_colour` argument.

_Note: Both legends together need more vertical space than the plate itself. For this example the height of the graphics device was therefore increased._

```{r well_borders_colour, dpi=260, fig.width=7, fig.height=6}
...
If you are only interested in the borders of the wells, you can provide a column that does not contain any values to the `value` argument. In this case no legend is shown for the `value` argument. Since every well contains `NA`, the `remove_na` argument has to be set to `FALSE` in order to not remove all of them. The `na_fill` argument determines the colour of the wells, which is white in the example below.

```{r well_borders_only, dpi=260, fig.width=7, fig.height=4}
